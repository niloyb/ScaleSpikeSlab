...
def inverse_precompute(
    Xt, d, prev_d, prev_inverse, tau0_inverse, tau1_inverse, tau0, tau1
):
    n = Xt.shape[1]
    p = Xt.shape[0]
    ...
    if no_swaps < min(no_slabs, no_spikes):
        ...
        inverse_update = Xt_update_prev_inverse.T @ (
            M_matrix_inverse @ Xt_update_prev_inverse
        )
        return prev_inverse - inverse_update
    elif no_slabs < no_spikes:
        Xt_update = Xt[slab_indices, :]
        Xt_update_prev_inverse = Xt_update @ tau0_inverse
        M_matrix = np.eye(no_slabs) / (tau1**2 - tau0**2) + (
            Xt_update_prev_inverse @ Xt_update.T
        )
        ...
        M_matrix = np.eye(no_spikes) / (tau0**2 - tau1**2) + (
            Xt_update_prev_inverse @ Xt_update.T
        )
        # Use (M_matrix+t(M_matrix))/2 to avoid numerical error, as M_matrix symmetric
        M_matrix_inverse = np.linalg.inv((M_matrix + M_matrix.T) / 2)
...
def update_beta(
    ...
    if min(no_swaps, no_spikes, no_slabs) < n:
        M_matrix_inverse = inverse_precompute(
            Xt, d, prev_d, prev_inverse, tau0_inverse, tau1_inverse, tau0, tau1
        )
    else:
        M_matrix_inverse = scipy.linalg.cho_solve(
            scipy.linalg.cho_factor(M_matrix, lower=True), np.eye(n)
        )
...
def update_sigma2(beta, z, tau0, tau1, a0, b0, X, y, u_crn=None):
    ...
    if u_crn is None:
        sigma2 = 1 / np.random.gamma(
            shape=((a0 + n + p) / 2), scale=(2 / (b0 + rss + beta2_d)), size=1
        )
    else:
        log_sigma2 = -np.log(
            gamma.ppf(q=u_crn, a=((a0 + n + p) / 2), scale=(2 / (b0 + rss + beta2_d)))
        )
...
def spike_slab_linear_kernel(
    ...
    beta_output = update_beta(
        ...
        tau0,
        tau1,
        u=random_samples["beta_u"],
        xi=random_samples["beta_xi"],
    )
    ...
    sigma2_new = update_sigma2(
        beta_new, z_new, tau0, tau1, a0, b0, X, y, u_crn=random_samples["sigma2_u"]
    )
    return {
        "beta": beta_new,
        "z": z_new,
        ...
        "prev_z": z,
        "prev_matrix": mat,
        "prev_inverse": mat_inverse,
    }


# spike_slab_linear
def spike_slab_linear(
    chain_length,
    X,
    y,
    tau0,
    ...
    Xt = X.T
    XXt = X @ Xt
    tau0_inverse = scipy.linalg.cho_solve(
        scipy.linalg.cho_factor(np.eye(n) + (tau0**2) * XXt, lower=True), np.eye(n)
    )
    tau1_inverse = scipy.linalg.cho_solve(
        scipy.linalg.cho_factor(np.eye(n) + (tau1**2) * XXt, lower=True), np.eye(n)
    )
    ...
    if rinit is None:
        ...
        prev_matrix = (
            np.eye(n)
            + XXt * (tau0**2)
            + (Xt_update.T @ Xt_update) * (tau1**2 - tau0**2)
        )
        prev_inverse = scipy.linalg.cho_solve(
            scipy.linalg.cho_factor(prev_matrix, lower=True), np.eye(n)
        )
    ...
    for t in range(chain_length):
        random_samples = {
            "beta_u": np.random.normal(size=p),
            "beta_xi": np.random.normal(size=n),
            "z_u": np.random.uniform(size=p),
            "sigma2_u": np.random.uniform(size=1),
        }
        new_state = spike_slab_linear_kernel(
            beta,
            z,
            sigma2,
            X,
    ...
    if store:
        ...
        return {
            "beta_ergodic_avg": beta_ergodic_sum / (chain_length - burnin),
...
def spike_slab_probit(
    chain_length, X, y, tau0, tau1, q, burnin=0, rinit=None, verbose=False, store=True
):
    n = X.shape[0]
    p = X.shape[1]
    Xt = X.T
    ...
    for t in range(chain_length):
        random_samples = {
            ...
            "beta_xi": np.random.normal(size=n),
            "z_u": np.random.uniform(size=p),
            "e_u": np.random.uniform(size=n),
        }

        new_state = spike_slab_probit_kernel(
            beta,
            z,
            e,
            X,
...
def matrix_full_logistic(Xt, d, sigma2tilde):
    n = Xt.shape[1]
    d_Xt = Xt / (d**0.5)[:, np.newaxis]
    return diagonal_inner_outer_prod(d_Xt.T @ d_Xt, 1 / (sigma2tilde**0.5)) + np.eye(
        n
    )


def matrix_precompute_logistic(
    Xt, d, sigma2tilde, prev_d, prev_matrix, prev_sigma2tilde, XXt, tau0, tau1
):
    p = Xt.shape[0]
    swap_indices = d != prev_d
    no_swaps = np.sum(swap_indices)
    ...
    if no_swaps == 0:
        ...
        return new_matrix
    if no_slabs == 0:
        new_matrix = diagonal_inner_outer_prod(
            (tau0**2) * XXt, 1 / sigma2tilde**0.5
        )
        np.fill_diagonal(new_matrix, new_matrix.diagonal() + 1)
        return new_matrix
    if no_spikes == 0:
        new_matrix = diagonal_inner_outer_prod(
            (tau1**2) * XXt, 1 / sigma2tilde**0.5
        )
        np.fill_diagonal(new_matrix, new_matrix.diagonal() + 1)
        return new_matrix
    ...
    if no_swaps < min(no_slabs, no_spikes):
        ...
        update_part = (Xt_update.T @ Xt_update) * (tau1**2 - tau0**2)
        new_matrix = diagonal_inner_outer_prod(
            XXt * (tau0**2) + update_part, 1 / sigma2tilde**0.5
        )
        np.fill_diagonal(new_matrix, new_matrix.diagonal() + 1)
    else:
        Xt_update = Xt[spike_indices, :]
        update_part = (Xt_update.T @ Xt_update) * (tau0**2 - tau1**2)
        new_matrix = diagonal_inner_outer_prod(
            XXt * (tau1**2) + update_part, 1 / sigma2tilde**0.5
        )
        np.fill_diagonal(new_matrix, new_matrix.diagonal() + 1)
    return new_matrix


def update_beta_logistic(
    z,
    sigma2tilde,
    X,
    Xt,
    ...
    M_matrix = matrix_precompute_logistic(
        Xt, d, sigma2tilde, prev_d, prev_matrix, prev_sigma2tilde, XXt, tau0, tau1
    )
    M_matrix_inverse = scipy.linalg.cho_solve(
        scipy.linalg.cho_factor(M_matrix, lower=True), np.eye(n)
...
def update_sigma2tilde_logistic(beta, z, tau0, tau1, a0, b0, X, y, u_crn=None):
    ...
    if u_crn is None:
        ...
        log_sigma2tilde = -np.log(
            gamma.ppf(q=u_crn, a=((a0 + 1) / 2), scale=(2 / (b0 + rss)))
        )
        sigma2tilde = np.exp(log_sigma2tilde)
    return sigma2tilde


def spike_slab_logistic_kernel(
    beta,
    z,
    e,
    sigma2tilde,
    ...
    beta_output = update_beta_logistic(
        z,
        sigma2tilde,
        X,
        Xt,
    ...
    a0 = nu
    b0 = w2 * nu
    sigma2tilde_new = update_sigma2tilde_logistic(
        beta_new,
        z_new,
        tau0,
        tau1,
        ...
        X,
        e_new,
        u_crn=random_samples["sigma2tilde_u"],
    )
    return {
        "beta": beta_new,
        "z": z_new,
        "e": e_new,
        "sigma2tilde": sigma2tilde_new,
        "prev_z": z,
...
def spike_slab_logistic(
    ...
    if rinit is None:
        ...
        Xt_update = Xt[prev_z == 1, :]
        update_part = (Xt_update.T @ Xt_update) * (tau1**2 - tau0**2)
        prev_matrix = diagonal_inner_outer_prod(
            XXt * (tau0**2) + update_part, 1 / sigma2tilde**0.5
        )
        np.fill_diagonal(prev_matrix, prev_matrix.diagonal() + 1)
    ...
    for t in range(chain_length):
        ...
        new_state = spike_slab_logistic_kernel(
            beta,
            z,
            e,
            sigma2tilde,
    ...
    if store:
        return {
            ...
            "z": z_samples,
            "e": e_samples,
            "sigma2tilde": sigma2tilde_samples,
        }
    else:
        return {
            "beta_ergodic_avg": beta_ergodic_sum / (chain_length - burnin),
            "z_ergodic_avg": z_ergodic_sum / (chain_length - burnin),
            "e_ergodic_avg": e_ergodic_sum / (chain_length - burnin),
            "sigma2tilde_ergodic_avg": sigma2tilde_ergodic_sum
            / (chain_length - burnin),
        }
